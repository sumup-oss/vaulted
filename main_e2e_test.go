...
// NOTE: Test the terraform workflow that is feasible when you're inheriting/migrating "legacy" encrypted terraform
...
// Tests intentionally expect blank stdout/stderr, even though it's wrong,
// to pass and later be able to correct to expected output.
func TestLegacyToV1TerraformWorkflow(t *testing.T) {
	t.Parallel()
	...
	// NOTE: Start of `1. legacy ini`
	legacyIniContent := []byte(`
[sectionExample]
myKey=example
...
`)
	...
	legacyIniFilePath := path.Join(tmpDir, "legacy.ini")
	err := osExecutor.WriteFile(legacyIniFilePath, legacyIniContent, 0644)
	require.Nil(t, err)

	legacyTfFilePath := path.Join(tmpDir, "legacy.tf")
	stdout, stderr, err := build.Run(
		"legacy",
		"ini",
		"--public-key-path",
	...
	legacyTfFileContent, err := osExecutor.ReadFile(legacyTfFilePath)
	require.Nil(t, err)
