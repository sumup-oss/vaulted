...
func TestNewLegacyCmd(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, "legacy", actual.Use)
	assert.Equal(t, "Legacy Proof-of-concept-phase commands", actual.Short)
	assert.Equal(t, "Legacy Proof-of-concept-phase commands that are now deprecated", actual.Long)
}

func TestLegacyCmd_Execute(t *testing.T) {
	t.Parallel()
